...
describe('parse', () => {
  test('leaflet', () => { expect(slippyTile.parse(TILE,
    'https://example.org/{z}/{x}/{y}.png').href).toBe(
  ...
  test('arcgis', () => { expect(slippyTile.parse(TILE,
    'https://example.org/tiles/256/{level}/{col}/{row}').href).toBe(
    `https://example.org/tiles/256/${z}/${x}/${y}`)
  })
  test('wmts', () => { expect(slippyTile.parse(TILE,
    'https://services.arcgisonline.com/arcgis/rest/services/World_Imagery/MapServer/WMTS/tile/1.0.0/World_Imagery/{Style}/{TileMatrixSet}/{TileMatrix}/{TileRow}/{TileCol}.jpg').href).toBe(
  ...
  test('wms', () => { expect(slippyTile.parse(TILE,
    'http://irs.gis-lab.info/?layers=landsat&SRS={proj}&WIDTH={width}&HEIGHT={height}&BBOX={bbox}').href).toBe(
